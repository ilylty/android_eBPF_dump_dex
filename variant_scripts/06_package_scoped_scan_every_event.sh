#!/system/bin/sh
set -eu
WORKDIR=/data/local/tmp/ebpfDumpDex/variants/06_package_scoped_scan_every_event
OUTDIR="$WORKDIR/out"
cd "$WORKDIR"
...
./dex_dump_bin -obj "$WORKDIR/dex_dump.bpf.o" -lib /apex/com.android.art/lib64/libdexfile.so -out "$OUTDIR" -max-dump 0 -scan >> "$WORKDIR/dexdump.log" 2>&1 &
